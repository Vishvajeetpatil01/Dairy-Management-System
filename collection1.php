...
if(isset($_POST['searchdata'])) {
    ...
    if ($result) {
        $milk_type = $result['mtype']; 

        // Fetch values based on milk type
        if ($milk_type == "Buffalo") {
            $table_name = "bfr";
        } elseif ($milk_type == "Cow") {
            $table_name = "cfr";
        } else {
            echo "Invalid milk type";
            exit;
        }

        // SQL query to fetch values
        $sql = "SELECT fat_rate, snf_rate FROM $table_name";
...
document.getElementById("customerbutton").addEventListener("click", function() {
    ...
    function confirmCollectionReport() {
        ...
        if (confirmation) {
            window.location.href = 'collectionreport1.php'; // Redirect if user selects "Yes"
        } else {
            // Do nothing or provide feedback to the user
        }
    }
    function crate() {
        window.location.href = 'cf.php';
    }
    function brate() {
        window.location.href = 'bf.php';
    }
    function report() {
    ...
    function newcustomer() {
        ...
        <form action="#" method="POST">
            <table border="1" cellspacing="6" cellpadding="6" align="center" class="table0" >
                ...
                <td><input type="text" name="rate" id="rate"  oninput="calculateTotal()" oninput="calculateRate()" readonly></td>
...
if(isset($_POST['insert'])){
    ...
    if(empty($_POST['search']) || empty($_POST['qty']) || empty($_POST['mtype']) || empty($_POST['fat']) || empty($_POST['snf']) || empty($_POST['rate']) || empty($_POST['total'])) {
        ...
        if(mysqli_num_rows($result) == 0){
            ...
            // Create a new table with the customer ID as the table name, if not exists
            ...
            mysqli_query($conn, "INSERT INTO $tableName(cdate, csession, mtype, qty, fat, snf, rate, total) VALUES ('$date','$session','$mtype','$qty','$fat','$snf','$rate','$total')");
...
$message = [
    ...
    "message" => "Collected Milk TYPE:$mtype, FAT:$fat, SNF:$snf, QTY:$qty, RATE:$rate TOTAL:$total." // Your SMS message content
];
