...
<h2 id="22" align="center">UPDATE COW FAT RATE</h2>

<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
    <table border="1" cellspacing="5" cellpadding="5" align="center">
        <tr><th id="21">COW Fat Rate</th><th>Cow Snf Rate</th></tr>
        <tr>
        ...
        <tr align="center">
            <td colspan="5">
                <input style="background-color: orange" type="submit" value="Update" size="5"/>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["rate"]) && isset($_POST["snf"])) {
    ...
    $snf = $_POST["snf"];
        if (!empty($brate)) {
            ...
            $check_query = mysqli_query($mysql, "SELECT * FROM cfr WHERE id = 1");
            if (mysqli_num_rows($check_query) > 0) {
                // Update the existing record
                mysqli_query($mysql, "UPDATE cfr SET fat_rate = '$brate', snf_rate = '$snf' WHERE id = 1");
            } else {
                // Insert a new record
...
<table border="1" cellspacing="5" cellpadding="5" align="center">
        <caption><strong>COW FAT RATE</strong></caption>
        <tr><th>COW FAT RATE</th><th>COW SNF RATE</th></tr>
        <?php
            $result3 = mysqli_query($mysql, "SELECT * FROM cfr ORDER BY id ");
