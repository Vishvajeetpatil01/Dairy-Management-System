...
<body>
    ...
    <h2 id="22" align="center">ADD BUFFALO RATE CHART</h2>

    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
        <table border="1" cellspacing="5" cellpadding="5" align="center">
            <tr><th>No</th><th id="19">Buffalo Fat</th><th id="20">S.N.F</th><th id="21">Buffalo Rate</th><th>Action</th></tr>
            <tr>
                <td><input type="text" name="no" id="no" size="20" maxlength="20"/></td>
                <td><input type="text" name="bfat" id="bfat" size="20" maxlength="20"/></td>
                <td><input type="text" name="bsnf" id="bsnf" size="20" maxlength="20"/></td>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if(isset($_POST["delete_btn"])) {
            $delete_no = $_POST["no"];
            mysqli_query($mysql, "DELETE FROM brate WHERE no='$delete_no'");
        } else {
            $no = $_POST["no"];
            $fat = $_POST["bfat"];
            $snf = $_POST["bsnf"];
            $rate = $_POST["brate"];
            if ($fat && $snf && $rate ) {
                mysqli_query($mysql, "INSERT INTO brate VALUES('$no','$fat','$snf','$rate')");
            }
        }
    ?>
    <table border="1" cellspacing="5" cellpadding="5" align="center">
        <caption><strong>BUFFALO-FAT-n-RATE-CHART-DETAILS</strong></caption>
        <tr><th>No</th><th>BUFFALO FAT</th><th>BUFFALO SNF</th><th>BUFFALO RATE</th><th>Action</th></tr>
        <?php
            $result3 = mysqli_query($mysql, "SELECT * FROM brate ORDER BY fat ");
